...
2. **Branch: can you settle every question in conversation?** If a question needs a runnable answer (state, business logic, a UI you have to see), detour through a prototype, bridged by **`/handoff`** in both directions (a prototype lives in its own directory, which is exactly what `/handoff` is for; see Phase boundaries):
   ...
   For frontend UI features, reach for **`/ui-prototype`** to explore 2–3 minimalist archetypes on throwaway mock data, freeze UX, and scaffold the consumer-driven contract specification (`api.ts` + `api.test.ts`). Then trigger **`/ui-review`** to audit screenshots and code rigor before backend wire-up.
...
- **`/ui-prototype`**: prototype interactive frontend UI with 2–3 minimalist archetypes on throwaway mock data before freezing UX, and scaffold consumer-driven API contract tests (`api.ts` + `api.test.ts`) on an in-memory transport seam for Phase 2 backend TDD.
- **`/ui-review`**: audit rendered frontend routes from live screenshots and source code across 5 axes (aesthetics, UX/navigability, design system, a11y, type rigidity & contracts) using parallel sub-agents before Phase 2 wire-up or PR merge.
