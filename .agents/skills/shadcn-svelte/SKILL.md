...
5. **Never edit generated primitives.** Files installed in `$lib/components/ui/` are upstream library code and must never be edited, modified, or patched. Always work around any tooling or linter issues externally.
6. **Always add components via CLI.** Never create component primitives manually. Always install them via `pnpm dlx shadcn-svelte@latest add -y <component>`.
